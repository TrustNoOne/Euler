...
object Euler93 extends EulerProblem {
  ...
  val Operations =
    List[(Double, Double) => Double](_ + _, _ - _, _ * _, (x, y) => if (y != 0) x / y else Double.MinValue)
  ...
  override def result() = {
    val longestConsecutives = for {
      ...
      c <- b + 1 to 9
      d <- c + 1 to 9
    } yield (longestConsecutive(a.toDouble, b.toDouble, c.toDouble, d.toDouble), s"$a$b$c$d")
