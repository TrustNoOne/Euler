...
object Euler54 extends EulerProblem {
  ...
  case class Hand(p1cards: Seq[Card]) {
    lazy val vals = p1cards.groupBy(_.value).mapValues(_.map(_.suit))
    lazy val suits = p1cards.groupBy(_.suit).mapValues(_.map(_.value))
  ...
  private def pow(n: Int, m: Int) = math.pow(n, m).toInt
  ...
  private def eval(h: Hand) = {
    ...
    val inverseCardCounts = cardCounts.groupBy(_._2).mapValues(_.keys.toList.sorted)
    ...
    val rankScore = cardCounts.values.toSeq.sorted match {
      ...
      case Seq(1, 2, 2) => // two pair
        2000000 + inverseCardCounts(1).sum +
          inverseCardCounts(1).zipWithIndex.map(x => pow(14, x._2 + 1) * x._1).sum
    ...
    val isStraight = rankScore < 1000000 && ((highestCard - lowestCard == 4) || (highestCard == 14 && cardCounts.dropRight(1).last._1 == 5))

    val straightScore = if (isStraight) 4000000 else 0
    val flushScore = if (isFlush) 5000000 else 0
    val royalScore = if (isStraight && isFlush && highestCard == 14) 9000000 else 0

    rankScore + flushScore + straightScore + royalScore
  }

  override def result = withResource("poker.txt") { src =>
