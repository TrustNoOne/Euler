...
object Euler59 extends EulerProblem {
  ...
  def decrypt(text: collection.Seq[Char], key: collection.Seq[Char]) = {
  ...
  override def result() = withResource("cipher1.txt") { src =>
    ...
    val decrypted = keys.view
      .map(k => decrypt(chars, k).toSeq)
      .find { text => text.containsSlice("this") && text.containsSlice("that") }
      .head

    // clear text:  decrypted.mkString
    decrypted.view.map(_.toInt).sum
  }
