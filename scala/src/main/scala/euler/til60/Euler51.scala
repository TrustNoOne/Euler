...
object Euler51 extends EulerProblem {
  ...
  def primePermutations(n: Int, addedDigitCnt: Int) = {
    ...
    digits.permutations.map { x =>
      (0 to 9).map { d => x.map { case -1 => d; case a => a } } map fromDigits filter isPrime
    } filterNot { _.isEmpty }
  }

  override def result() = {
    val res = LazyList.from(100) flatMap { primePermutations(_, 3) } dropWhile (xs => xs.size < 8 || xs.head < 100000)
    res.head.head
  }
}
