...
object Euler66 extends EulerProblem {
  ...
  override def result = {
    val res = (2 to 1000) filterNot isPerfectSquare map { d =>
      (d, solveDiophantine(d).get._1)
    }
    res.maxBy(_._2)._1
  }
  ...
  def solveDiophantine(n: Int) = {
    val convIt = new ConvergentsIterator(Iterator.continually(1),
                                         new SqrContinuedFractionIterator(n))
