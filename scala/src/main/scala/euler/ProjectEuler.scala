...
object ProjectEuler extends App {
  ...
  val eulerProblems =
    Iterator.from(1).map(eulerProblem).takeWhile(_.isDefined).flatten

  val (totalElapsed, _) = elapsed {
    eulerProblems.zipWithIndex foreach { case (prob, n) =>
  ...
  private def eulerProblem(n: Int)(implicit m: Mirror): Option[EulerProblem] = {
    ...
    val pkg = 10 + (n - 1) / 10 * 10
    try {
      val module = m.staticModule(s"euler.til$pkg.Euler" + n)
      val eulerProblem =
        m.reflectModule(module).instance.asInstanceOf[EulerProblem]
      Some(eulerProblem)
    } catch {
      case e: ScalaReflectionException if e.msg.startsWith("object euler.til") && e.msg.endsWith("not found.") => None
    }
  }
