...
object Euler80 extends EulerProblem {
  ...
  def sqrDigits(x: Int, numDigits: Int): Seq[Int] = {
    ...
    @annotation.tailrec
    def loop(c: BigInt, p: BigInt, res: Seq[Int]): Seq[Int] = if (res.size == numDigits) res else {
      //Determine the greatest digit x such that x(20p + x) <= c
      val x = Iterator.from(0).find { x =>
        val nextX = x + 1
        nextX * (20 * p + nextX) > c
      }.get
      val y = x * (20 * p + x)
  ...
  def pairsIterator(x: Int) = {
    val xStr = x.toString
    val xDigits = if (xStr.size % 2 == 0) xStr.map(_.toString.toInt) else ('0' +: xStr).map(_.toString.toInt)
