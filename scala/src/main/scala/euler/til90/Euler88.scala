...
object Euler88 extends EulerProblem {
  ...
  def intCombinations(n: Int,
                      limit: Int,
                      k: Int = 2,
                      currProd: Int = 1): Seq[(List[Int], Int, Int)] = {
    (k to limit * 2 / currProd).flatMap {
      case x if n > 1 =>
        intCombinations(n - 1, limit, x, currProd * x).map(t =>
          (x :: t._1, t._2 * x, t._3 + x))
        ...
        .foldLeft(Map[Int, Int]()) {
          case (map, (list, prod, sum)) =>
            ...
            if (k > kMax) map
              ...
              map.get(k).fold(map.updated(k, prod)) { value =>
