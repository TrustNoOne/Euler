...
object Euler93 extends EulerProblem {
  ...
  val Operations = List[(Double, Double) => Double](_ + _, _ - _, _ * _, (x, y) => if (y != 0) x / y else Double.MinValue)
  ...
  override def result = {
    val longestConsecutives = for {
      ...
      c <- b + 1 to 9
      d <- c + 1 to 9
    } yield (longestConsecutive(a, b, c, d), s"$a$b$c$d")
  ...
  def longestConsecutive(a: Double, b: Double, c: Double, d: Double) = {
    val all = for {
      ...
      res <- Seq(op3(op2(op1(xs(0), xs(1)), xs(2)), xs(3)), op3(op1(xs(0), xs(1)), op2(xs(2), xs(3))))
    } yield res
    ...
    val targets = all.filter(x => x > 0 && x == math.rint(x)).toSet.toList.sorted
    loop(targets).toInt
  }
